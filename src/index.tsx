...
function ImageSafe({ srcs, alt, className }: { srcs: string[]; alt: string; className?: string }) {
...
const CATALOG: Record<string, any> = Object.fromEntries(PRODUCTS.map((p) => [p.id, p]));

// ---- Currency helpers ----
const FX = { MXN: 18.0, EUR: 0.92 };
const two = (n: number) => n.toFixed(2);
const priceLine = (usd: number) => `$${two(usd)} USD • $${two(usd * FX.MXN)} MXN • €${two(usd * FX.EUR)} EUR`;
...
function useCart() {
  ...
  const normalize = (list: any[]) => {
    const counts: Record<string, number> = { sg1: 0, ln1: 0, bd1: 0 };
    list.forEach((i) => {
  ...
  const safeSet = (updater: any) => {
    setItems((prev) => {
      ...
      const beforeCounts = prev.reduce((m: any, i) => ((m[i.id] = (m[i.id] || 0) + i.qty), m), {});
      const after = normalize(before);
      const afterCounts = after.reduce((m: any, i: any) => ((m[i.id] = (m[i.id] || 0) + i.qty), m), {});
  ...
  const remove = (id: string) => safeSet((prev: any[]) => prev.filter((i) => i.id !== id));
  const inc = (id: string) => safeSet((prev: any[]) => prev.map((i) => (i.id === id ? { ...i, qty: i.qty + 1 } : i)));
  const dec = (id: string) =>
    safeSet((prev: any[]) =>
...
// Responsive nav: desktop tabs + mobile dropdown
function Nav({ current, onNav }: { current: string; onNav: (p: string) => void }) {
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`rounded-full px-3 py-1 text-sm border ${
              current === t.id ? "bg-[var(--brand-accent)] text-white" : "bg-white"
            }`}
          >
            {t.label}
          </button>
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`block w-full text-left px-4 py-2 text-sm hover:bg-neutral-50 ${
                current === t.id ? "font-semibold text-[var(--brand-primary)]" : "text-neutral-800"
              }`}
            >
              {t.label}
            </button>
...
// ---- Pages ----
function HomePage({ cart, catalogRef }: { cart: any; catalogRef: any }) {
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-white p-4">
        <p className="text-sm">
          <strong>Offer:</strong> Free shipping on orders $100+ and our{" "}
          <span className="brand-heading">Solaris Bundle</span> for $100 (save $10).
        </p>
        <button
          className="brand-btn"
          onClick={() => {
  ...
  const staff = [
    {
      ...
      title: "President",
      blurb:
        "Founded Solaris after spotting a gap in affordable, premium interchangeable eyewear. Greg focuses on strategy, operations, and keeping the brand aligned with its original college born vision. He has led the team for three years and drives every major product release from concept to launch.",
      img: "/images/greg.png",
    },
    {
      ...
      blurb:
        "Samantha oversees brand identity, campaigns, and partnerships that put Solaris on the map. With years of experience in digital marketing and lifestyle branding, she makes sure our frames and lenses are shown in real world settings that customers actually live in.",
      img: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=600&auto=format",
    },
    {
      name: "Abby Dulin",
      title: "VP of Customer Service & Outreach",
      blurb:
        "Abby leads customer support and community outreach, ensuring every Solaris buyer feels heard and taken care of. She sets service standards, builds feedback loops into product updates, and manages our growing user community across North America and abroad.",
      img: "https://images.unsplash.com/photo-1529626455594-4ff0802cfb7e?q=80&w=600&auto=format",
    },
    {
      name: "Ryder Edwards",
      title: "VP of Technology",
      blurb:
        "Ryder manages the Solaris website, digital tools, and the backend systems that keep orders moving smoothly. With a background in e-commerce and product tech, he focuses on reliability, speed, and a clean customer experience from first click to delivery.",
      img: "/images/ryder.png",
    },
    {
      name: "Troy Johnson",
      title: "VP of Products & Service",
      blurb:
        "Leads product design and service standards. With 7 years in premium eyewear and outdoor gear, Troy makes sure every Solaris frame and lens performs in real world use.",
      img: "/images/troy1.png",
    },
  ];
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <p className="text-neutral-700 mb-4">
        Solaris began as a college project among classmates in an E-Business class taught by Professor Kevin Jetton.
        What started as a shared passion for design and entrepreneurship quickly evolved into a thriving company.
        Our founders combined their backgrounds in technology, marketing, customer service, and leadership to create stylish,
        innovative eyewear built for modern life. Three years later, Solaris continues to grow, maintaining its commitment to quality
        craftsmanship, inclusive design, and sustainable practices.
      </p>
      <p className="text-neutral-700 mb-6">
        Founded officially in 2022, Solaris blends classic style with modern lens technology.
        We design for all faces and all walks of life with an inclusive fit program. Meet our leadership team below.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {staff.map((s) => (
          <div key={s.name} className="overflow-hidden rounded-2xl border bg-white">
            <ImageSafe
              srcs={[s.img, "https://images.unsplash.com/photo-1603415526960-f7e0328b27d8?q=80&w=600&auto=format"]}
              alt={s.name}
    ...
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-2xl border bg-white p-4">
          ...
          <p className="text-sm">
            Solaris Eyewear Co.<br />
            1234 Commerce Ave, Suite 200<br />
        ...
        >
          ...
          <p className="text-xs text-neutral-600 mb-2">
            Click send to open your email client addressed to our webmaster.
          </p>
          <button type="submit" className="brand-btn">
            Email {COMPANY.emailWebmaster}
          </button>
        </form>
      </div>
    </section>
...
function FAQPage() {
  const faqs = [
    ...
    { q: "Do you ship internationally?", a: "Yes, we ship across North America for $20 USD and Europe/Asia for $30 USD." },
    {
      ...
      a: "We currently do not, but plan to in the future. Our upcoming frame only line will remain compatible with all Solaris lenses.",
    },
  ];
  ...
  const rows = [
    {
      ...
      city: "Dallas, TX",
      quote:
        "Love how easily I can swap out the lenses. Each replacement comes in a soft cloth pouch with different colors and polarizations.",
    },
    {
      name: "Marco",
      city: "Austin, TX",
      quote:
        "I surf every weekend and switch between shades depending on sunlight. The magnetic lenses make it easy.",
    },
    {
      name: "Priya",
      city: "San Antonio, TX",
      quote:
        "When my original lens cracked, I replaced it in seconds with the backup pair from the bundle. Still my favorite shades.",
    },
  ];
...
function SpanishPage() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <p className="text-neutral-700 mb-4">
        Gafas de sol de diseño clásico con tecnología moderna. Envío gratis en pedidos de $100 o más.
      </p>
    ...
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="rounded-2xl border bg-white p-4 space-y-3 text-sm text-neutral-800">
        <p>
          ...
          This site collects no personal data except what you voluntarily send via email links.
        </p>
        <p>
          <span className="font-semibold">Cookies:</span> No tracking cookies are set.
        </p>
        <p>
          <span className="font-semibold">Third parties:</span> We do not sell or share your data with a third party.
          All payment processing is done in house through Stripe.
        </p>
        <p>
          <span className="font-semibold">Your rights:</span> To delete your personal data, contact our webmaster at{" "}
          <a href={`mailto:${COMPANY.emailWebmaster}`} className="underline">
            {COMPANY.emailWebmaster}
          </a>
          .
        </p>
        <h2 className="brand-heading text-2xl mt-6">Security Practices</h2>
        <ul className="list-disc pl-6 space-y-1">
          <li>HTTPS enforced across solaris.fit with HSTS policy.</li>
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <p className="mt-6 text-sm text-neutral-600">
        For privacy and security details, see our combined{" "}
        <button
          onClick={() => window.dispatchEvent(new CustomEvent("solaris-nav", { detail: "PRIVACY" }))}
          className="underline"
        >
          Privacy Policy
        </button>
      ...
      <div className="mx-auto flex max-w-6xl justify-between px-4 py-6 text-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-3">
          <span>
            ...
            <a href={COMPANY.baseUrl} className="hover:underline">
              Solaris.fit
            </a>
          </span>
          <button
            onClick={() => window.dispatchEvent(new CustomEvent("solaris-nav", { detail: "TERMS" }))}
        ...
        <div className="flex items-center gap-4">
          <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" aria-label="Twitter">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-blue-500 hover:opacity-75">
              <path d="M22.46 6c-.77.35-1.6.59-2.46.7a4.27 4.27 0 0 0 1.88-2.36 8.59 8.59 0 0 1-2.72 1.04 4.24 4.24 0 0 0-7.22 3.86 12.04 12.04 0 0 1-8.74-4.43 4.23 4.23 0 0 0 1.31 5.65 4.17 4.17 0 0 1-1.92-.53v.05a4.25 4.25 0 0 0 3.4 4.16 4.22 4.22 0 0 1-1.91.07 4.25 4.25 0 0 0 3.96 2.94A8.52 8.52 0 0 1 2 19.54a12.03 12.03 0 0 0 6.53 1.92c7.84 0 12.12-6.5 12.12-12.13 0-.18 0-.36-.01-.54A8.65 8.65 0 0 0 24 5.1a8.44 8.44 0 0 1-2.54.7z" />
            </svg>
          </a>
          <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-blue-700 hover:opacity-75">
              <path d="M22 12a10 10 0 1 0-11.5 9.9v-7h-2v-2.9h2V9.5c0-2 1.2-3.2 3-3.2.9 0 1.8.16 1.8.16v2h-1c-1 0-1.3.63-1.3 1.3v1.5h2.4l-.38 2.9h-2v7A10 10 0 0 0 22 12z" />
            </svg>
          </a>
          <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-5 h-5 text-pink-500 hover:opacity-75">
              <path d="M7.5 2A5.5 5.5 0 0 0 2 7.5v9A5.5 5.5 0 0 0 7.5 22h9a5.5 5.5 0 0 0 5.5-5.5v-9A5.5 5.5 0 0 0 16.5 2h-9zM12 7a5 5 0 1 1 0 10 5 5 0 0 1 0-10zm0 1.8a3.2 3.2 0 1 0 0 6.4 3.2 3.2 0 0 0 0-6.4zm4.75-.9a1.05 1.05 0 1 1 0 2.1 1.05 1.05 0 0 1 0-2.1z" />
            </svg>
          </a>
          <a href="https://pinterest.com" target="_blank" rel="noopener noreferrer" aria-label="Pinterest">
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-5 h-5 text-red-600 hover:opacity-75">
              <path d="M12 2C6.48 2 2 6.48 2 12c0 4.1 2.5 7.6 6.08 9.12-.09-.77-.17-1.95.03-2.8.18-.77 1.16-4.9 1.16-4.9s-.3-.6-.3-1.5c0-1.4.8-2.45 1.8-2.45.84 0 1.25.63 1.25 1.38 0 .84-.54 2.1-.82 3.27-.23.98.49 1.78 1.46 1.78 1.75 0 3.1-1.84 3.1-4.48 0-2.34-1.68-3.98-4.08-3.98-2.78 0-4.4 2.08-4.4 4.23 0 .84.32 1.75.72 2.24.08.1.1.18.08.27-.08.3-.26.95-.3 1.08-.05.17-.18.23-.42.14-1.56-.63-2.55-2.58-2.55-4.15 0-3.37 2.45-6.48 7.07-6.48 3.7 0 6.58 2.63 6.58 6.13 0 3.68-2.32 6.64-5.55 6.64-1.08 0-2.1-.56-2.46-1.22l-.67 2.55c-.24.92-.9 2.07-1.33 2.78.99.31 2.03.48 3.1.48 5.52 0 10-4.48 10-10S17.52 2 12 2z" />
            </svg>
          </a>

          <a href={`mailto:${COMPANY.emailWebmaster}`} className="hover:underline text-neutral-600">
            {COMPANY.emailWebmaster}
          </a>
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const subtotal1 = 85;
      const shipping1 = subtotal1 >= 100 ? 0 : 14.95;
      console.assert(shipping1 === 14.95, "Ship 14.95 at 85");
      const remaining1 = Math.max(0, 100 - subtotal1);
      console.assert(remaining1 === 15, "Remain 15 at 85");
      const progress1 = Math.min(100, Math.round((subtotal1 / 100) * 100));
      console.assert(progress1 === 85, "Progress 85 percent at 85");
      const subtotal2 = 100;
      const shipping2 = subtotal2 >= 100 ? 0 : 14.95;
      console.assert(shipping2 === 0, "Free ship at 100");
      const tax = +(100 * 0.0825).toFixed(2);
      console.assert(tax === 8.25, "Tax 8.25 on 100");
      const pl = priceLine(10);
      console.assert(pl === "$10.00 USD • $180.00 MXN • €9.20 EUR", "Currency line");
      console.groupEnd();
    } catch (e) {
      console.error("Runtime checks failed", e);
    }

    const onNav = (e: any) => {
      const target = e?.detail;
      if (target && (PAGES as any)[target]) setPage(target);
    };
      ...
      <header className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-3 py-2 md:px-4 md:py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className="brand-btn text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-2"
              onClick={() => setShowCart((v) => !v)}
            >
              Cart ({cart.items.reduce((n: number, i: any) => n + i.qty, 0)})
            </button>
          </div>
        </div>
      </header>
      ...
      {showCart && (
        <div className="fixed right-4 top-16 z-50 w-[22rem] rounded-2xl border bg-white shadow-xl">
          <div className="flex items-center justify-between p-4">
            ...
            <button className="text-sm opacity-70" onClick={() => setShowCart(false)}>
              Close
            </button>
          </div>
              ...
              cart.items.map((i: any) => (
                <div key={i.id} className="mb-3 flex items-center justify-between border-b pb-3 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <button className="rounded border px-2" onClick={() => cart.dec(i.id)}>
                      -
                    </button>
                    <button className="rounded border px-2" onClick={() => cart.inc(i.id)}>
                      +
                    </button>
                    <button className="rounded border px-2" onClick={() => cart.remove(i.id)}>
                      Remove
                    </button>
                  </div>
                </div>
          ...
          <div className="space-y-1 p-4 text-sm">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span className="tabular-nums">${cart.totals.subtotal.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Shipping</span>
              {cart.totals.subtotal >= 100 ? <span className="brand-badge">FREE</span> : <span className="tabular-nums">$14.95</span>}
            </div>
            <div className="flex justify-between">
              <span>Tax</span>
              <span className="tabular-nums">${cart.totals.tax.toFixed(2)}</span>
            </div>
            <div className="flex justify-between border-t pt-2 font-semibold">
              <span>Total</span>
              <span className="tabular-nums">${cart.totals.total.toFixed(2)}</span>
            </div>
            {cart.autoBundled && (
              <div className="mt-2 rounded-lg border bg-white p-2 text-xs">
                ...
                <button className="ml-2 underline" onClick={cart.clearAutoBundled}>
                  dismiss
                </button>
              </div>
            )}
        ...
        <div className="w-full border-b bg-white/90">
          <div className="mx-auto max-w-6xl px-4 py-2 text-sm">
            {remainingForFree > 0 ? (
              ...
              <div className="flex items-center gap-2">
                <span className="brand-badge">Shipping: FREE</span>
                <span>Congrats, you unlocked free shipping.</span>
              </div>
            )}
          </div>
        </div>
